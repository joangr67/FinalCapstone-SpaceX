...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        ...
        # Contamos los valores con 'value_counts' y devuelve una serie 
        # reset_index lo convierte en un dataframe con dos columnas (índice, valor del conteo)
        outcome_counts = filtered_df['class'].value_counts().reset_index()
        # renombra las columnas del dataframe para mayor claridad
        outcome_counts.columns = ['Outcome', 'Count']
        # reemplaza los valores (0 y 1) por las etiquetas 'Failure' y 'Success'
        outcome_counts['Outcome'] = outcome_counts['Outcome'].map({1: 'Success', 0: 'Failure'})

        fig = px.pie(outcome_counts, 
                    values='Count', 
                    names='Outcome', 
                    title=f'Launches at {entered_site}: Succes vs Failures',
...
def update_scatter_chart(selected_site, payload_range):
    ...
    if selected_site == 'ALL':
        fig = px.scatter(filtered_df, x='Payload Mass (kg)', y='class',
                         ...
                         title='Payload and Launch Success Relationship (All Sites)',
                         labels={'class': 'Successful launch'})
    #Si seleccionamos un lugar de lanzamiento filtramos por ese lugar y dibujamos
        ...
        fig = px.scatter(site_df, x='Payload Mass (kg)', y='class',
                         color='Booster Version Category',
                         title=f'Payload and success relationship in {selected_site}',
                         labels={'class': 'Successful launch'})
    return fig
